...
export type UserRole = "admin" | "manager" | "collaborateur" | "client";
...
export const usersService = {
  ...
  async changeUserRole(agentId: string, newRole: UserRole): Promise<Agent> {
    const response = await fetch(
      `${API_BASE_URL}/users/${agentId}/change-role`,
      {
        method: "PATCH",
        headers: getAuthHeaders(),
    ...
    if (!response.ok) throw new Error("Erreur changement rôle");
    return response.json();
  },

  async promoteToAdmin(agentId: string): Promise<Agent> {
    const response = await fetch(
      `${API_BASE_URL}/users/${agentId}/promote-to-admin`,
      {
        method: "PATCH",
        headers: getAuthHeaders(),
      }
    ...
    if (!response.ok) throw new Error("Erreur promotion admin");
    return response.json();
  },
  ...
  async searchUsers(params: {
    ...
    const response = await fetch(
      `${API_BASE_URL}/users/search?${queryParams}`,
      {
        headers: getAuthHeaders(),
      }
    );
    ...
    if (!response.ok) {
      ...
      // Si c'est une 404, retourner un tableau vide au lieu de throw
      if (response.status === 404) {
        console.warn(
          "Route /users/search non trouvée, retournant tableau vide"
        );
        return [];
      }
