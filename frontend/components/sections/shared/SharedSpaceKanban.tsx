...
interface SharedSpaceKanbanProps {
  space: Space;
  members: any[]; // Liste des utilisateurs invités dans l'espace
}
...
export default function SharedSpaceKanban({ space, members }: SharedSpaceKanbanProps) {
  ...
  const [expandedTasks, setExpandedTasks] = useState<Record<string, boolean>>(() => {
    if (typeof window !== "undefined") {
      const saved = localStorage.getItem(`expanded_tasks_shared_${space._id}`);
  ...
  const { data: tasks = [], isLoading } = useQuery({
    queryKey: ["sharedTasks", space._id],
    queryFn: () => sharedTaskService.getSpaceTasks(space._id),
  });

  // Synchronisation des colonnes avec les tâches
  ...
  const collisionDetectionStrategy: CollisionDetection = useCallback(
    (args) => {
      ...
      if (allCollisions.length > 0) lastOverId.current = allCollisions[0].id;
  ...
  const handleUpdateTask = async (taskId: string, data: any) => {
    try {
      const updatedTask = await sharedTaskService.update(space._id, taskId, data);
