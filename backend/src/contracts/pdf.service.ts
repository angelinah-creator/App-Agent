...
export class PdfService {
  ...
  private async generatePdfFromTemplate(templateName: string, user: UserDocument): Promise<Buffer> {
    ...
    try {
      ...
      if (user.signature?.url) {
        userSignatureBase64 = await this.urlToBase64(user.signature.url);
      } else {
        // Signature par défaut si non disponible
        userSignatureBase64 = await this.getDefaultSignature();
      }
  ...
  /**
   * Retourner une signature par défaut
   */
  private async getDefaultSignature(): Promise<string> {
